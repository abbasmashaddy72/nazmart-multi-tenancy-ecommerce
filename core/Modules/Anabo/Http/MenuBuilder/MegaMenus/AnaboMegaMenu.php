...
namespace Modules\Anabo\Http\MenuBuilder\MegaMenus;


use Modules\Blog\Entities\Blog;
use Plugins\MenuBuilder\MegaMenuBase;
use Plugins\MenuBuilder\MegaMenus\BlogCategory;

class AnaboMegaMenu extends MegaMenuBase
{
...
//                $output .= '<li><a href="'.route('frontend.blog.single',$post->slug).'">' . $post->title . '</a></li>'."\n";
            ...
            $output .= '</ul>'."\n";
            $output .= '<h2 class="btn btn-primary">'."\n";
            $output .= 'External Menu'."\n";
            $output .= '</h2>'."\n";
            $output .= '</div>'."\n";
            $output .= '</div>'."\n";
        }
